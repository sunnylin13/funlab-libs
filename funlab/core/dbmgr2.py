...
class DbMgr:
    def __init__(self, conf:Config) -> None:
        self.config=conf.get_section_config('database', case_insensitive=True)
        if not self.config and conf.get('url', case_insensitive=True):
            self.config = Config(conf)
        else:
            raise Exception('No database configuration data in provided Config object.')
    ...
    def get_db_url(self)->str:
        try:
            ...
            raise NoDBUrlDefined(f"No 'database' or related url is defined in config.toml. Check!") from e
